...
const MemberCard = ({ member, isAdmin, onDelete, onEdit, variant = 'standard' }) => {
    ...
    const containerClasses = isPresident
        ? "relative group bg-[#66CBFF]/10 p-8 rounded-[30px] shadow-sm hover:shadow-[0_0_30px_rgba(102,203,255,0.6)] transition-all duration-300 flex flex-col md:flex-row items-center md:items-start gap-8 border-none w-full"
        : "relative group bg-white p-6 rounded-[20px] shadow-sm hover:shadow-[0_0_25px_rgba(102,203,255,0.5)] transition-all duration-300 flex flex-col sm:flex-row items-center sm:items-start gap-6 border border-gray-100 w-full";

    const imageClasses = isPresident
        ? "bg-center bg-no-repeat bg-cover rounded-full w-40 h-40 md:w-56 md:h-56 border-[6px] border-[#66CBFF]/20 shadow-lg object-cover"
        : "bg-center bg-no-repeat bg-cover rounded-full w-28 h-28 md:w-32 md:h-32 border-[3px] border-[#66CBFF]/20 shadow-md object-cover";
                ...
                {isEditing ? (
                    <div className="flex flex-col gap-3 w-full">
                        ...
                        <div className="flex gap-2 justify-end mt-2">
                            ...
                            <button onClick={handleSave} className="px-3 py-1 text-xs bg-[#66CBFF] text-white rounded-md">Salva</button>
                        </div>
                    </div>
                ) : (
                    <>
                        ...
                        <div className={`flex flex-wrap items-center gap-4 ${isPresident ? 'justify-center md:justify-start' : 'justify-center sm:justify-start'}`}>
                            {/* Instagram */}
                            {member.instagram && (
                                <a href={`https://instagram.com/${member.instagram.replace('@', '').replace('https://instagram.com/', '')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-gray-500 hover:text-[#66CBFF] transition-colors group">
                                    ...
                                    <span className="text-xs font-medium max-w-[120px] truncate">{member.instagram}</span>
                                </a>
                            )}

                            {/* Facebook */}
                            {member.facebook && (
                                <a href={`https://facebook.com/${member.facebook.replace('https://facebook.com/', '')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-gray-500 hover:text-[#66CBFF] transition-colors group">
                                    <div className="w-8 h-8 rounded-lg bg-[#66CBFF]/10 flex items-center justify-center group-hover:bg-[#66CBFF] transition-all duration-200 shadow-sm shrink-0">
                                        {icons.facebook}
                                    </div>
